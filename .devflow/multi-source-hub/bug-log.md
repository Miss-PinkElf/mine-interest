...
| 问题现象 | 原因 | 解决方案与状态 |
| --- | --- | --- |
| 图片/表情包未持久保存 | 插件只记录 `file` + `url`（腾讯 CDN，含 `rkey` 鉴权），不下载。AstrBot 的 `data/temp/media_image_*` 临时文件一分钟内即被清理 | **未解决**。2026-09-20 手动验证可下载（1.2 MB GIF，HTTP 200），证明方案可行；插件自动落盘待实施，等用户定体积策略 |
