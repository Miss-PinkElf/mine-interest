...
## 2026-07-13 14:28:23 +08:00 - 全 tasks 完成后的边界决策

- 决策：将 `spec/tasks.md` 全部勾选视为 **V1 代码闭环完成**；真实 FFmpeg/STT/MediaPipe/云端 HTTP/Playwright 浏览器环境作为 **加深项**，不回写为未完成 tasks。
- 备选：把适配层实现视为 tasks 未完成并保持红灯。
- 原因：计划与验收以可替换协议、测试与审核闭环为主；真实重模型依赖本机硬件、密钥与样本，适合下一阶段独立验证。
- 影响：新会话应优先替换适配器并跑 benchmark，而不是重做脚手架。

## 2026-07-13 14:28:23 +08:00 - 会话提交策略

- 决策：用户授权后，每完成一个 task/阶段即 commit；收尾只提交本 mission 相关文档与代码。
- 原因：上下文过长需新开对话，提交颗粒度便于回滚与恢复。

## 2026-07-13 15:05:44 +08:00 - 下一优先：JobRunner + 可先 Fake STT

- 决策：新对话优先实现上传后自动最小管线（可 Fake STT 写片段），再换真引擎；不重做 tasks 脚手架。
- 备选：先接真 FFmpeg/真 STT 再串管线。
- 原因：用户已理解 task≠成品；最短路径是让审核页有字、能导出，使既有 API/UI/模型插座立刻可用。
- 影响：完成度从「平台壳」跳到「可演示闭环」。

## 2026-07-13 15:05:44 +08:00 - Mac 定位与 Windows 目标

- 决策：Mac 用于开发联调与轻量/远程 STT；正式重模型与 CUDA 验收在 Windows+RTX。
- 原因：本机无 ffmpeg、无 CUDA；设计目标机为 Win+3080。
